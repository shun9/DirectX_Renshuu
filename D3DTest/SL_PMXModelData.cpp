...
bool PMXModelData::LoadModel(char* fileName)
{
	//ファイルを開く
	FILE* file;
...
/// PMXのデータを読み込む
/// </summary>
bool PMXModelData::Load(FILE * file)
{
	//pmxの読み込み
...
bool PMXModelData::LoadHeader(FILE* file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadModelInfo(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadVertex(FILE * file)
{
	if (file == NULL)
	{
		return false;
	...
	m_vertex.boneWeightInfo.resize(m_vertex.count);
	for (int i = 0; i < m_vertex.count; i++)
	{
		...
		//追加UV読み込み
		for (int j = 0; j < m_header.data[HEADER_DATA_2_0::ADD_UV]; j++)
		{
			fread_s(&m_vertex.boneWeightInfo[i].addUv[i], sizeof(Vec4), sizeof(Vec4), 1, file);
		}

		//ウェイト変形方式読み込み
		fread_s(&m_vertex.boneWeightInfo[i].weightType, sizeof(PMXByte), sizeof(PMXByte), 1, file);

		//ウェイトの読み込み
		if (!ReadBoneWeight(&m_vertex.boneWeightInfo[i].boneWeight, m_vertex.boneWeightInfo[i].weightType, file))
			return false;

		//エッジ倍率
		fread_s(&m_vertex.boneWeightInfo[i].edgeScale, sizeof(float), sizeof(float), 1, file);
	}
	return true;
}
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadFace(FILE * file)
{
	if (file == NULL)
	{
		return false;
	...
	{
		fread_s(&m_face.info[i].index, sizeof(int), size, 1, file);
	}
	return true;
}
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadTexture(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadMaterial(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadBone(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadMorph(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadDisplayFrame(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadRigidBody(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadJoint(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
/// <param name="file">オープンしたファイルポインタ</param>
/// <returns>成功でtrue</returns>
bool PMXModelData::LoadSoftBody(FILE * file)
{
	if (file == NULL)
	{
		return false;
...
bool PMXModelData::ReadName(PMXTextBuf * textBuf, FILE * file)
{
...
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadText(std::string* buf, int byte, PMXByte encode, FILE * file)
{
	if (encode == PMX_UTF16)
	{
		int size = byte / 2;
...
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadBoneWeight(PMXBoneWeight* buf, PMXByte type, FILE * file)
{
	PMXByte size = m_header.data[HEADER_DATA_2_0::BONE_INDEX];
	switch (type)
	{
...
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadBoneFlag(PmxBoneInfo * buf, unsigned short flag, FILE * file)
{
	PMXByte boneIndexSize = m_header.data[HEADER_DATA_2_0::BONE_INDEX];
...
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadGroupMorphOffset(vector<GroupMorphOffset>* buf,int count, FILE * file)
{
	PMXByte morphIndexSize = m_header.data[HEADER_DATA_2_0::MORPH_INDEX];
...
/// <param name="count">作成数</param>
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadVertexMorphOffset(vector<VertexMorphOffset>* buf, int count, FILE * file)
{
...
/// <param name="count">作成数</param>
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadBoneMorphOffset(vector<BoneMoptOffset>* buf, int count, FILE * file)
{
	PMXByte boneIndexSize = m_header.data[HEADER_DATA_2_0::BONE_INDEX];
	for (int i = 0; i < count; i++)
	{
...
/// <param name="count">作成数</param>
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadUVMorphOffset(vector<UVMorphOffset>* buf, int count, FILE * file)
{
	PMXByte vertexIndexSize = m_header.data[HEADER_DATA_2_0::VERTEX_INDEX];
	for (int i = 0; i < count; i++)
	{
...
/// <param name="count">作成数</param>
/// <param name="file">読み込むファイル</param>
bool PMXModelData::ReadMaterialMorphOffset(vector<MaterialMorphOffset>* buf, int count, FILE * file)
{
...
/// <param name="count">作成数</param>
/// <param name="file">読み込むファイル</param>
bool PMXModelData::MakeMorphOffset(PMXMorphInfo* buf, PMXByte type, int count, FILE * file)
{
	switch (type)
	{
